...
def configure_document(doc: Document, preset: str, running_title: str, doc_code: str):
    ...
    normal.paragraph_format.line_spacing = (
        1.10 if preset == "standard_business_brief" else 1.25
    )
    ...
    heading_specs = {
        "Heading 1": (
            16,
            BLUE,
            16 if preset == "standard_business_brief" else 18,
            8 if preset == "standard_business_brief" else 10,
        ),
        "Heading 2": (
            13,
            BLUE,
            12 if preset == "standard_business_brief" else 14,
            6 if preset == "standard_business_brief" else 7,
        ),
        "Heading 3": (
            12,
            DARK_BLUE,
            8 if preset == "standard_business_brief" else 10,
            4 if preset == "standard_business_brief" else 5,
        ),
    }
    ...
    for name in ("List Bullet", "List Number"):
        ...
        style.paragraph_format.left_indent = Inches(
            0.5 if preset == "standard_business_brief" else 0.375
        )
        style.paragraph_format.first_line_indent = Inches(
            -0.25 if preset == "standard_business_brief" else -0.188
        )
        style.paragraph_format.space_after = Pt(
            8 if preset == "standard_business_brief" else 4
        )
        style.paragraph_format.line_spacing = (
            1.167 if preset == "standard_business_brief" else 1.25
        )
    ...
    for style_name, size, color, bold in [
        ...
        set_style_font(
            ...
            color,
            bold,
            "Consolas" if style_name == "Code Block" else FONT_WESTERN,
        )
...
def set_table_geometry(
    table, widths_dxa: Sequence[int], indent_dxa: int = TABLE_INDENT_DXA
):
    if sum(widths_dxa) != CONTENT_WIDTH_DXA:
        raise ValueError(
            f"Table widths must total {CONTENT_WIDTH_DXA}, got {sum(widths_dxa)}"
        )
    table.autofit = False
    tbl_pr = table._tbl.tblPr
...
def add_callout(
    doc: Document, label: str, text: str, fill: str = LIGHT_GREEN, accent: str = GREEN
):
...
def add_para(
    doc: Document, text: str, bold_lead: str | None = None, style: str | None = None
):
...
def add_bullets(
    doc: Document, items: Sequence[str | tuple[str, str]], numbered: bool = False
):
...
def build_prd() -> Document:
    doc = Document()
    configure_document(
        doc, "standard_business_brief", "标策 AI 产品需求文档", "BC-AI-PRD-001"
    )
    add_masthead(
        doc,
    ...
    add_bullets(
        ...
        [
            (
                "唯一规则源：",
                "评标条件、权重和公式来自当前项目招标文件，不使用全国统一的“万能评分表”。",
            ),
            (
                "先门槛后评分：",
                "资格和实质性要求先按通过/不通过审查，有效投标才进入详细评审。",
            ),
            (
                "双向文档匹配：",
                "招标要求与公司资质、业绩、人员和技术材料逐条映射，输出缺口和原文位置。",
            ),
            (
                "多智能体竞演：",
                "A 代表我方，B/C/D 模拟不同竞争策略，确定性规则裁判负责淘汰、计分和排序。",
            ),
            (
                "不承诺中标：",
                "系统提高决策质量与可解释性，最终结论仍需项目负责人和评标委员会依法确认。",
            ),
        ],
    )
    add_page_break(doc)
    ...
    add_bullets(
        ...
        [
            (
                "项目专属：",
                "每次建项都重新解析规则；模板只用于初始化，不替代正式招标文件。",
            ),
            (
                "确定性裁判：",
                "硬门槛、限价、计分公式和排序由规则引擎执行，生成式智能体不能改写。",
            ),
            (
                "证据优先：",
                "能力、证书或业绩只有在招标文件列为要求且具备规定证明材料时才算满足或得分。",
            ),
            ("版本可追溯：", "招标更正、人工纠错和证据替换都生成版本并触发重新评估。"),
            (
                "人工在环：",
                "高风险条款、低置信度抽取、异常低价和最终决策必须有人确认。",
            ),
    ...
    flow = [
        (
            "创建项目与上传文件。",
            "上传招标主文件、采购需求、评分办法、附件和更正公告。",
        ),
        (
            "抽取并锁定规则。",
            "识别资格条件、实质性要求、评分项、权重、公式、最高限价和证明要求。",
        ),
        (
            "接入公司材料。",
            "上传或复用营业执照、资质、证书、业绩、人员、技术和服务材料。",
        ),
        ("执行证据匹配。", "逐条输出已满足、部分满足、缺失、置信度及双向原文定位。"),
        (
            "作出 Bid/No-Bid。",
            "先判断门槛、工期、资源、成本和利润可行性，再决定是否参与。",
        ),
        (
            "运行多智能体竞演。",
            "模拟我方与不同竞争策略，规则裁判执行淘汰、计分、排名和概率映射。",
        ),
        (
            "优化报价并出报告。",
            "按中标概率、期望利润或稳健收益目标搜索报价，生成审批与封标待办。",
        ),
    ]
    add_bullets(
        doc,
        [
            (f"步骤 {idx + 1}：{title}", f" {body}")
    ...
    add_table(
        ...
        [
            [
                "FR-01",
                "项目与文件接入",
                "批量接收招标文件、附件和公司材料，展示类型、大小、版本与处理状态",
                "P0",
                "部分实现",
            ],
            [
    ...
    add_table(
        ...
        [
            ...
            [
                "文档解析",
                "定时动画 + 项目模拟规则",
                "OCR、版面解析、表格识别、LLM 抽取和校验",
            ],
            [
                "公司证据",
                "模拟材料列表和匹配结果",
                "企业证据库、有效期、主体、全文/向量索引",
            ],
            [
                "评标规则",
                "三个项目模板和确定性前端函数",
                "规则 DSL、版本管理、公式测试和人工确认",
            ],
            [
                "竞争模拟",
                "四个固定画像 + 概率映射",
                "历史数据校准、场景抽样、置信区间和模型评估",
            ],
            [
                "协作审计",
                "本地状态与 JSON 导出",
    ...
    add_table(
        ...
        [
            [
                "安全",
                "传输与静态加密、租户隔离、最小权限、敏感文件水印",
                "通过安全评审与渗透测试",
            ],
            [
                "隐私",
                "文件用途告知、保留期限、删除能力、模型训练隔离",
                "可追踪数据生命周期",
            ],
            [
                "可靠性",
                "解析任务可重试、幂等、断点续传，规则引擎结果可复现",
                "关键任务成功率 ≥ 99.5%",
            ],
            [
                "性能",
                "上传即时反馈，常规项目在可接受时间内形成规则初稿",
                "100 页 PDF 初稿目标 ≤ 5 分钟",
            ],
            [
                "可解释性",
                "每条判断含来源、证据、置信度、公式和版本",
                "关键结论追溯覆盖率 100%",
            ],
            [
                "可访问性",
                "键盘操作、可见焦点、屏幕阅读器标签、颜色非唯一编码",
                "核心流程满足 WCAG 2.1 AA",
            ],
        ],
        [1500, 5000, 2860],
    ...
    add_table(
        ...
        [
            [
                "北极星",
                "从项目创建到形成可复核 Bid/No-Bid 建议的中位时长",
                "按项目、采购类别分层统计",
            ],
    ...
    add_table(
        ...
        [
            [
                "V0.2 · 当前",
                "前端流程、模拟上传解析、三类项目模板、四方竞演和报告",
                "完成产品评审与交互确认",
            ],
            [
                "V0.5 · MVP",
                "真实文件存储/解析、规则确认、企业证据库、基础账号权限",
                "5–10 个历史项目回放通过",
            ],
            [
                "V0.8 · 试点",
                "任务协作、审计、规则 DSL、历史竞争画像与评估",
                "真实试点项目稳定运行",
            ],
            [
                "V1.0 · 生产",
                "安全合规、监控、容灾、模型评测、企业集成与运维",
                "生产验收与制度落地",
            ],
        ],
        [1600, 4700, 3060],
    ...
    add_table(
        ...
        [
            [
                "招标条款抽取错误",
                "误判资格或评分",
                "原文定位、置信度、双模型/规则校验、人工锁定",
                "高",
            ],
            [
                "扫描件和复杂表格识别失败",
                "规则缺失",
                "OCR 质量检测、表格专用解析、失败告警",
                "高",
            ],
            [
                "公司证据过期或主体不符",
    ...
    add_table(
        ...
        [
            [
                "资格审查",
                "判断投标人主体、信用、资质和其他准入条件是否符合。通常为通过制。",
            ],
            [
                "符合性审查",
                "判断报价、签章、有效期、★ 条款等是否实质响应。通常为通过制。",
            ],
...
def build_frontend_design() -> Document:
    doc = Document()
    configure_document(
        doc, "compact_reference_guide", "标策 AI 前端设计文档", "BC-AI-FE-001"
    )
    add_masthead(
        doc,
        ...
        [
            ...
            (
                "线上 Demo",
                "https://bid-agent-lab-20260806.breezy-toad-2233.chatgpt.site",
            ),
        ],
    )
    add_callout(
    ...
    add_table(
        ...
        [
            [
                "规则与门槛",
                "这个项目到底怎么评？",
                "规则画像、资格/符合性清单、评分结构、公式、四方门槛矩阵",
                "回到证据或模拟",
            ],
            [
                "评分证据",
                "我方为什么能拿这些分？",
                "证据成熟度、评分项—材料—页码—预测得分—缺口",
                "提高成熟度、创建补缺任务",
            ],
            [
                "决策报告",
                "管理层应该如何决策？",
                "Bid/No-Bid、建议报价、依据、封标动作、风险边界",
                "复制摘要、导出 JSON/正式报告",
            ],
        ],
        [1600, 2500, 3560, 1700],
    ...
    add_table(
        ...
        [
            [
                ...
                "全局规则、模板和竞手",
                "Project API + RuleVersion",
            ],
            [
                "文档",
                "tenderFiles / companyFiles",
                "上传区与数量徽标",
                "Document API + 对象存储",
            ],
            [
                "解析",
                "parseState / progress / stage",
                "流水线与结果可见性",
                "ParseJob 轮询/推送",
            ],
            [
                "证据匹配",
                "DocumentMatch[] / gapsClosed",
                "覆盖率、门槛和矩阵",
                "EvidenceMatch API",
            ],
            [
                "我方约束",
                "ourBid / cost / minMargin",
                "利润、可行区间和优化",
                "Scenario 草稿",
            ],
            [
                "模型假设",
                "readiness / marketPressure / objective",
                "得分、竞手和推荐价",
                "Scenario 参数",
            ],
            [
    ...
    add_table(
        ...
        [
            ...
            [
                "DocumentDropzone",
                "文件选择、拖拽、校验、列表",
                "kind, accept, files, onAdd, onRemove",
                "招标/公司上传",
            ],
            [
                "AnalysisPipeline",
                "展示任务阶段和进度",
                "status, progress, stage, agents",
                "文档解析",
            ],
            [
                "RuleSummaryCard",
    ...
    add_table(
        ...
        [
            [
                "app/page.tsx",
                "模板数据、规则函数、状态、全部页面 JSX",
                "拆分 features/documents、rules、simulation、report",
            ],
            [
                "app/globals.css",
                "完整视觉系统和响应式样式",
                "拆分 token、layout、component 样式或 CSS Modules",
            ],
            ["app/layout.tsx", "中文元数据与社交预览", "根据请求 Host 生成绝对 OG URL"],
            [
                "前端内存状态",
                "文件元数据、解析动画、匹配和模拟",
                "服务端持久化 + Query 缓存 + URL/项目状态",
            ],
            [
                "确定性函数",
                "evaluateScenario、优化搜索、覆盖率",
                "提取为共享规则包并增加单元/性质测试",
            ],
    ...
    add_table(
        ...
        [
            ...
            [
                "GET",
                "/projects/{id}/rule-versions/latest",
                "获取当前规则版本",
    ...
    add_table(
        ...
        [
            ...
            [
                ...
                "解析任务状态变化",
                "projectId, duration, stage, errorCode",
            ],
            ["match_gap_reviewed", "用户查看或确认缺口", "requirementKind, status"],
            [
                "analysis_applied_to_simulation",
                "将匹配结果带入竞演",
                "coverage, hardGapCount",
            ],
            [
                "scenario_parameter_changed",
                "调整报价/成本/目标",
                "field, previous, next",
            ],
    ...
    add_table(
        ...
        [
            ...
            [
                "社交预览图",
                "public/bid-strategy-social.png",
                "四条策略路径汇入规则决策",
            ],
        ],
        [2300, 2650, 4410],
